...
def sync_data():
    print("Starting comprehensive database synchronization (v3)...")
    
    # 1. Initialize DataLoader
    data_loader.load_data()
    print(f"DataLoader loaded: {len(data_loader.qualifications)} quals, {len(data_loader.jobs)} jobs")
    ...
    try:
        ...
        # 5. Populate All Majors with categories
        print("Syncing detailed majors with categories...")
        db.execute(text("TRUNCATE TABLE major RESTART IDENTITY CASCADE"))
        
        # Map integrated_code -> integrated_name
        integrated_name_map = {}
        integrated_major_file = os.path.join("dataset", "integrated_major1.csv")
        if os.path.exists(integrated_major_file):
            with open(integrated_major_file, mode='r', encoding='utf-8-sig') as f:
                reader = csv.DictReader(f)
                for row in reader:
                    integrated_name_map[row['integrated_code']] = row['integrated_name']

        # Map detail_id -> integrated_code
        ...
        if os.path.exists(detail_major_file):
            with open(detail_major_file, mode='r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    if m_name:
                        i_code = detail_to_integrated.get(d_id)
                        category = integrated_name_map.get(i_code, "기타")
                        major = Major(major_name=m_name, major_category=category)
                        db.add(major)
                        major_count += 1
            db.commit()
        print(f"Major table populated. Total: {major_count}")
        ...
        # Re-use groupings for speed or just re-load
        detail_id_to_name = {}
        with open(detail_major_file, mode='r', encoding='utf-8-sig') as f:
            reader = csv.DictReader(f)
            for row in reader:
                ...
                for row in reader:
                    ...
                    if qual_id and i_code in integrated_to_details:
                        for d_name in integrated_to_details[i_code]:
                            ...
                            mapping_count += 1
            db.commit()
        print(f"Major-Qualification mappings synced. Total: {mapping_count}")
        ...
        for qual_id, jobs in data_loader.qual_job_mapping.items():
            for job in jobs:
                ...
                job_map_count += 1
        db.commit()
        print(f"Qualification-Job mappings synced. Total: {job_map_count}")

        print("SUCCESS: Database is now fully synchronized with CSV data and categories.")
